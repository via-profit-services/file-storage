...
create database services_file_storage;
grant all privileges on database services_file_storage to services;
```

3. Попробуйте выполнить подключение:
```bash
psql --host=localhost --username=services --dbname=services_file_storage --password
```
